...
class WindowsSon(WindowsParent):
    ...
    def front_page(self):
        ...
        self.global_image_file = PhotoImage(file=r"C:\Users\chuan\Downloads\tropical-10201.gif")  # create an image variable
        image = canvas.create_image(0, 0, anchor="center", image=self.global_image_file)  # put the image variable into canvas
    ...
    def win_for_append(self):
        ...
        def receive_save_file():
            ...
            if(condition1):
                ...
                # sent message
                custom_name_message = messagebox.showwarning(title="Warning", message="name can not be digit, symbol or blank")
                ...
                try:
                    for line in f1:  # line(str)
                        for key in json.loads(line)[0].keys():
                            if(receipt_num_entry.get() == key):  # if the receipt is the same with the receipt in the file
                                message = messagebox.showwarning(title="Warning", message="same receipt number found")  # show warning
                                f1.close()
                                return 0  # end the function
                except:
                    f1.close()
            ...
            #  sent a message of warning if condition doesn't fit
                ...
                # sent message
                receipt_number_message = messagebox.showwarning(title="Warning", message="postive number can't be symbol, char or blank")
                return 0  # end the function

            item_name = item_hired_entry.get()
            ...
            if(number_hired_entry.get().isdigit() and 0 < int(number_hired_entry.get()) <= 500):
                ...
                # sent message
                number_hired_message1 = messagebox.showwarning(title="Warning", message="number are between 1~500 and number can't be symbol, char or blank")
                return 0  # end the function
        ...
        item_hired_entry = Entry(new_window)  # create an entry for hired item
        item_hired_entry.place(x=180, y=135)  # define the coordinate for the Entry of hired number
    ...
    def win_for_search(self):
            ...
            if (receipt_number.isdigit() and int(receipt_number) > 0):
                ...
                try:
                    try:
                        ...
                        message = messagebox.showwarning(title="Warning", message="Can not find history")  # send a message for warning
                        return 0  # end the function
                    for line in f:
                        data.append(json.loads(line))
                        f.close()
                        for new_line in data:
                            for key in new_line[0].keys():  # key is the receipt number in the file
                ...
                message = messagebox.showwarning(title="Warning", message="postive number can't be symbol, char or blank")
                return 0
        ...
        def new_win_for_search(receipt_number, my_list):
            ...
            second_window.geometry("300x300")  # define the size of the new window
            second_window.minsize(width=300, height=300)  # define the minimum size of the new window
            second_window.maxsize(width=300, height=300)  # define the maximum size of the new window
            ...
            receipt_number_label = Label(second_window, text=f"receipt num: {receipt_number}", font=("Arial", 18), bg="burlywood2")
            receipt_number_label.place(x=30, y=130)  # define the coordinate of the label

            # create a label for item hired
            item_hired_label = Label(second_window, text=f"item name: {my_list[1]}", font=("Arial", 18), bg="burlywood3")
            item_hired_label.place(x=30, y=160)  # define the coordinate of the label

            # create a label for hired number
            number_hired_label = Label(second_window, text=f"num hired: {my_list[2]}", font=("Arial", 18), bg="burlywood4")
        ...
        def delete_row():
            ...
            if (receipt_number.isdigit() and int(receipt_number) > 0):
                try:
                    ...
                    message = messagebox.showwarning(title="Warning", message="Can not find history")  # sent a warning message
                    return 0  # end the function
